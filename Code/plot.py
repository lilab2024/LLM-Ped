...
ax.plot(angles, radii, color='darkorange', linewidth=2, label=f'AUC = {roc_auc:.2f}')
ax.fill(angles, radii, color='orange', alpha=0.3)

# 图形修饰
ax.set_title(' ', va='bottom', fontsize=20)
ax.legend(loc='upper right')
ax.set_theta_zero_location('N')  # 将0度设为顶部
ax.set_theta_direction(-1)       # 顺时针

# 设置坐标轴刻度字号
ax.tick_params(axis='both', labelsize=17)  # 增大角度轴和半径轴的刻度字体
ax.legend(loc='upper right', fontsize=17, bbox_to_anchor=(1.1, 1.1))


plt.savefig('ROC_polar.png', dpi=300)
plt.show()
